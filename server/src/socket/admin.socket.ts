...
export function setupAdminEvents(io: Server, socket: Socket) {
 ...
 socket.on('admin:kick', async (data: { targetUser
 ...
 socket.on('admin:teleport', async (data: { targetUser
 const admin = socket.data.user;
 ...
 socket.on('admin:summon', async (data: { targetUser
 const admin = socket.data.user;
 ...
 socket.on('admin:freeze', async (data: { targetUser
 const admin = socket.data.user;
 ...
 socket.on('admin:unfreeze', async (data: { targetUser
 const admin = socket.data.user;
 ...
 io.receiver(`room:${roomId}`).emit('room_alert', {
 message: data.message,
 sender: admin.username,
 role: admin.role,
 timestamp: new Date()
 ...
 if (data.invisible) {
 // Disparaître
 io.receiver(`room:${roomId}`).emit('player_left', {
 userId: admin.id
 });
 } else {
 // Réapparaître
 io.receiver(`room:${roomId}`).emit('player_joined', {
 user: admin,
 ...
 io.receiver(`room:${data.roomId}`).emit('room_locked', {
 by: admin.username,
 timestamp: new Date()
 });
